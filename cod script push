    ...
    5)
      ...
      if [ "$PUSH_ANSWER" == "yes" ]; then
        ...
        if [ $? -eq 0 ]; then
          ...
          echo -e "${YELLOW}Push failed. Trying to pull first and then push again...${RESET}"
          git pull --rebase
          if [ $? -eq 0 ]; then
            git push
            if [ $? -eq 0 ]; then
              echo -e "${GREEN}Changes pushed after pull.${RESET}"
            else
              echo -e "${YELLOW}Push still failed. Please resolve conflicts manually.${RESET}"
            fi
          else
            echo -e "${YELLOW}Pull failed. Please resolve conflicts manually.${RESET}"
          fi
        fi
      else
        echo "Push canceled."
